...
Input: csv with wall pier ID, XSection ID, direction, length, thickness
Process: Create a FAST-TCF script using input data
Output: FAST-TCF script: wallforces.inp
Python Version: 3.6
Script Version: 0.0.1
"""


wallinputs = r'C:\Users\kevin.chen\Documents\_Kevin\01 Jobs\08 SCVMC\LS-DYNA\scripts\piers.csv'
outfile = r'C:\Users\kevin.chen\Documents\_Kevin\01 Jobs\08 SCVMC\LS-DYNA\scripts\wallforces.inp'
...
for i in range(nwalls):
  ...
  walllength = inputs[i,6]
  wallthick = inputs[i,7]
  
  scriptadd = ['section '+section+' force '+direction+' tag curve_'+str(i*4+1),
               'section '+section+' force z tag curve_'+str(i*4+2),
               'operation div curve_'+str(i*4+1)+' 4448 tag curve_'+str(i*4+3),
               'operation div curve_'+str(i*4+2)+' 4448 tag curve_'+str(i*4+4),
               'delete curve_'+str(i*4+1)+' curve_'+str(i*4+2),
               'label curve_'+str(i*4+3)+'  '+wallID+' in-plane shear (kips)',
               'label curve_'+str(i*4+4)+'  '+wallID+' axial (kips)',
               'stylec curve_'+str(i*4+3)+' '+colors[(2*i)%len(colors)]+',default,solid,circle,1',
               'stylec curve_'+str(i*4+4)+' '+colors[(2*i+1)%len(colors)]+',default,solid,circle,1',
               '#']
